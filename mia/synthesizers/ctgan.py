...
from mia.synthesizers.utils_opacus import _updated_create_or_extend_grad_sample
...
class CTGANSynthesizer(BaseSynthesizer):
    ...
    def fit(self, train_data, categorical_columns=tuple(), ordinal_columns=tuple(), args=None):
        ...
        if args.dp:
            opacus.supported_layers_grad_samplers._original_create_or_extend_grad_sample = opacus.supported_layers_grad_samplers._create_or_extend_grad_sample
            opacus.supported_layers_grad_samplers._create_or_extend_grad_sample = _updated_create_or_extend_grad_sample
        ...
        if args.dp:
            if args.dp_type == 'sgd': # DP-SGD
                ...
                sigma = torch.tensor(args.max_grad_norm * args.noise_multiplier, device=self.device)
                batch = torch.tensor(self.batch_size, device=self.device)
                for parameter in discriminator.parameters():
                    parameter.register_hook(
                        lambda grad: grad + (1 / batch) * sigma
                        * torch.randn(parameter.shape).to(self.device)
                    )
        if args.dp:
            if args.dp_type == 'sgd': # DP-SGD
                ...
                optimizerG = optim.RMSprop(self.generator.parameters(), lr=0.001, alpha=0.99)
            optimizerD = optim.RMSprop(discriminator.parameters(), lr=0.001, alpha=0.99)
        else:
            optimizerG = optim.Adam(
                self.generator.parameters(), lr=2e-4, betas=(0.5, 0.9), weight_decay=self.l2scale)
            optimizerD = optim.Adam(discriminator.parameters(), lr=2e-4, betas=(0.5, 0.9))

        if args.dp and args.dp_type == 'sgd': # DP_SGD:
        ...
        for i in range(self.epochs):
            ...
            for id_ in range(steps_per_epoch):
                ...
                if not args.dp or (args.dp and args.dp_type=='sgd'):
                    loss_d = -torch.mean(y_real) + torch.mean(y_fake) # sgd and not dp
                    if not args.dp:
    ...
    def sample_pbb(self, fakez):
        ...
        c1 = self.cond_generator.sample_zero(fakez.shape[0])
        c1 = torch.from_numpy(c1)
        fakez = torch.cat([fakez, c1], dim=1)

        fake = self.generator(fakez)
        fakeact = apply_activate(fake, output_info).detach().numpy()

        return self.transformer.inverse_transform(fakeact, None)