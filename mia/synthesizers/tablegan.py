...
from mia.constants import CATEGORICAL
from mia.synthesizers.base import BaseSynthesizer
from mia.synthesizers.utils import TableganTransformer
...
class TableganSynthesizer(BaseSynthesizer):
    ...
    def fit(self, data, categorical_columns=tuple(), ordinal_columns=tuple(), args=None):
        sides = [4, 8, 16, 24, 32]
        for i in sides:
            if i * i >= data.shape[1]:
                self.side = i
        ...
        data = torch.from_numpy(train_data.astype('float32')).to(self.device)
        dataset = TensorDataset(data)
        loader = DataLoader(dataset, batch_size=int(self.batch_size), shuffle=True, drop_last=True)

        layers_D, self.layers_G, layers_C = determine_layers(
            self.side, self.random_dim, self.num_channels)

        self.generator = Generator(self.transformer.meta, self.side, self.layers_G).to(self.device)
    ...
    def sample_pbb(self, fakez):
        ...
        noise = torch.zeros(fakez.shape[0], self.random_dim, 1, 1)
        fake = self.generator(noise).detach().numpy()
